    ...
    if (fin.is_open()) {
        ...
        while (!fin.eof()) {
            ...
            if (name.compare("circle") == 0) {
                ...
                str[c] == '(' && str[c + 1] == '(' ? bracketsStatus = 2
                                                   : bracketsStatus = 0;
                c += 2;
            } else {
                std::cout << str << std::endl;

                std::cout << "Error at column 0: expected 'circle', 'triangle'"
                          << std::endl;
                exit(0);
            }
            ...
            if (bracketsStatus == 0) {
                ...
                std::cout << "Error at column " << c << ": expected '('"
                          << std::endl;
                exit(0);
            }
            ...
            if (name.compare("triangle") == 0) {
                if (pointsArray[0].compare(pointsArray[pointsCount]) != 0) {
                    std::cout << str << std::endl;
                    std::cout << "Error at column " << c
                              << " first and last points not same "
                              << std::endl;
                    exit(0);
                }
            }
